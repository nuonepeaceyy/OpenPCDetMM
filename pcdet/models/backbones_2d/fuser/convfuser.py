...
class ConvFuserV2(nn.Module):
    ...
    def forward(self,batch_dict):
        ...
        with torch.no_grad():  # 确保CLIP模型不参与梯度计算
            category_caption = batch_dict['category_caption'].tolist()
            cat_input_ids = self.clip_processor(category_caption, return_tensors="pt", padding=True).input_ids
...
class ConvFuserV3(nn.Module):
    def __init__(self,model_cfg) -> None:
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(257, 256, 3, padding=1, bias=False),
            nn.BatchNorm2d(out_channel),
            nn.ReLU(True)
        )
        self.clip_model = CLIPModel.from_pretrained("/mnt/32THHD/zsj/project/mllms/clip")
    ...
    def forward(self,batch_dict):
        img_bev = batch_dict['spatial_features_img']
        lidar_bev = batch_dict['spatial_features']
        cat_bev_vi = torch.cat([img_bev, lidar_bev], dim=1)
        mm_bev_vi = self.conv(cat_bev_vi)
        ...
        cat_bev = torch.cat([mm_bev_vi, text_feat], dim=1)
        mm_bev = self.conv3(cat_bev)
